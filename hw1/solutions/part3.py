...
def plot_h(fig, ax, px, py, slice, h_fn):
    ...
    H = h_fn(X.reshape(-1, 13)).reshape(len(px), len(py))

    px = px.detach().cpu().numpy()
    py = py.detach().cpu().numpy()
    H = H.detach().cpu().numpy()

    im = ax.pcolormesh(px, py, H.T)
    fig.colorbar(im)
    ax.contour(px, py, H.T, colors='k', levels=[0])
...
def plot_and_eval_xts(fig, ax, x0, u_ref_fn, h_fn, dhdx_fn, gamma, lmbda, nt, dt):
    ...
    for xt in xts:
        ax.plot(xt[:, 0], xt[:, 1], color='r')

    is_safe = safe_mask(x0)
    is_fail = torch.any(failure_mask(xts.reshape(-1, 13)).reshape(len(x0), nt), dim=1)
    return (torch.sum(is_fail[is_safe])/torch.sum(is_safe)).item()