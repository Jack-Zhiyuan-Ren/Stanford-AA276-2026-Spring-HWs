...
def optimal_control(x, dVdx):
    ...
    PXi, PYi, PZi, QWi, QXi, QYi, QZi, VXi, VYi, VZi, WXi, WYi, WZi = [i for i in range(13)]
    PX, PY, PZ, QW, QX, QY, QZ, VX, VY, VZ, WX, WY, WZ = [x[:, i] for i in range(13)]
    dVdPX, dVdPY, dVdPZ, dVdQW, dVdQX, dVdQY, dVdQZ, dVdVX, dVdVY, dVdVZ, dVdWX, dVdWY, dVdWZ = [dVdx[:, i] for i in range(13)]

    F_opt = 20*torch.sign(dVdVX*(QW*QY + QX*QZ) + dVdVY*(QY*QZ - QW*QX) + dVdVZ*(0.5 - torch.pow(QX, 2) - torch.pow(QY, 2)))
    ax_opt = 8*torch.sign(dVdWX)
    ay_opt = 8*torch.sign(dVdWY)
    az_opt = 4*torch.sign(dVdWZ)
    u_opt = torch.stack((F_opt, ax_opt, ay_opt, az_opt), dim=1)
    return u_opt
...
def hamiltonian(x, dVdx):
    ...
    ham = torch.matmul(dVdx.unsqueeze(1), f(x).unsqueeze(2) + torch.matmul(g(x), u_opt.unsqueeze(2))).squeeze()
    return ham
...
def hji_vi_loss(x, l, V, dVdt, dVdx):
    ...
    """
    return torch.abs(torch.minimum(dVdt + hamiltonian(x, dVdx), l - V))